...
try {
    ...
    DISCORD_LOG(LS_INFO) << "InitLeafNode: got ciphersuite";
    ...
    if (!transientKey) {
        if (!signingKeyId_.empty()) {
            ...
            DISCORD_LOG(LS_INFO) << "InitLeafNode: generating transientKey";
            transientKey = std::make_shared<::mlspp::SignaturePrivateKey>(
              ::mlspp::SignaturePrivateKey::generate(ciphersuite));
            DISCORD_LOG(LS_INFO) << "InitLeafNode: generated transientKey";
        }
    }

    selfSigPrivateKey_ = transientKey;

    DISCORD_LOG(LS_INFO) << "InitLeafNode: CreateUserCredential";
    auto selfCredential = CreateUserCredential(selfUserId, protocolVersion_);

    DISCORD_LOG(LS_INFO) << "InitLeafNode: generating selfHPKEPrivateKey";
    selfHPKEPrivateKey_ =
      std::make_unique<::mlspp::HPKEPrivateKey>(::mlspp::HPKEPrivateKey::generate(ciphersuite));
    DISCORD_LOG(LS_INFO) << "InitLeafNode: generated selfHPKEPrivateKey";

    DISCORD_LOG(LS_INFO) << "InitLeafNode: making LeafNode";
    selfLeafNode_ =

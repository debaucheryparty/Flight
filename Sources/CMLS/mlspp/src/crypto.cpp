...
tls_signature_scheme(Signature::ID id)
{
  switch (id) {
    ...
    default:
      throw InvalidParameterError("Unsupported algorithm: unknown CipherSuite ID " + std::to_string(static_cast<uint16_t>(id)));
  }
}
...
CipherSuite::get() const
{
  ...
  switch (id) {
    ...
    default:
      throw InvalidParameterError("Unsupported ciphersuite in CipherSuite::get for id=" + std::to_string(static_cast<uint16_t>(id)));
  }
}

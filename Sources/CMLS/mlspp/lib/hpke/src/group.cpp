...
struct ECKeyGroup : public EVPGroup
{
    ...
    const typed_unique_ptr<BIGNUM>& priv) const
  {
    ...
    if (name == nullptr) {
      throw std::runtime_error("Unsupported algorithm: name is nullptr for curve_nid=" + std::to_string(curve_nid));
    }
...
private:
  ...
  static int group_to_nid(Group::ID group_id)
  {
    switch (group_id) {
      ...
      default:
        throw std::runtime_error("Unsupported algorithm in group_to_nid for group_id=" + std::to_string(static_cast<int>(group_id)));
    }
  }
  ...
  uint8_t bitmask() const
  {
    switch (id) {
      ...
      default:
        throw std::runtime_error("Unsupported algorithm in bitmask for group_id=" + std::to_string(static_cast<int>(id)));
    }
  }
};
...
private:
  ...
  static int group_to_evp(Group::ID group_id)
  {
    switch (group_id) {
      ...
      default:
        throw std::runtime_error("Unsupported algorithm in group_to_evp for group_id=" + std::to_string(static_cast<int>(group_id)));
    }
  }
};

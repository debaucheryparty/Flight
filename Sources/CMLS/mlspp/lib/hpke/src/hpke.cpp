...
#endif
    default:
      throw std::runtime_error("Unsupported algorithm in select_kem for id=" + std::to_string(static_cast<uint16_t>(id)));
  }
}
...
select_kdf(KDF::ID id)
{
  switch (id) {
    ...
    default:
      throw std::runtime_error("Unsupported algorithm in select_kdf for id=" + std::to_string(static_cast<uint16_t>(id)));
  }
}
...
select_aead(AEAD::ID id)
{
  switch (id) {
    ...
    default:
      throw std::runtime_error("Unsupported algorithm in select_aead for id=" + std::to_string(static_cast<uint16_t>(id)));
  }
}
